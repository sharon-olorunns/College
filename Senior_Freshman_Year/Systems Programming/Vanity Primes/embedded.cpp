...
class VanityPrimeHandler : public CivetHandler
{
  public:
	...
	handleGet(CivetServer *server, struct mg_connection *conn)
	{
		...
		while(!findPrime && attempts<MAX_ATTEMPTS){
			attempts++;
			...
			mbedtls_mpi_write_string( &mpi, HEX_BASE, buf, VANITY_PRIME_LENGTH, &outputLength );
		...
		if(findPrime){
			...
			mg_printf(conn, "400 Bad Request\nYou have exceeded the attempt limit (%i)", MAX_ATTEMPTS);
		}
